...
#==============================================================================
	...
	BTPeer.__init__(self, maxpeers, serverport)
        ...
        super(FilerPeer, self).setflag(False)
	...
	handlers = {LISTPEERS : self.__handle_listpeers,
            ...
            INITIALIZE: self.__handle_initialize,
    ...
    def __handle_initialize(self, peerconn, data):
        '''signal = data
        print "received data"
        if signal == 'INIT':
            BTPeer.setProcessOn( self )
        peerconn.senddata(REPLY, 'Success')'''
        try:
            print "recieved init"
            #print data
            ip, port, flag = data.split(",")
            #print ip, port, flag
            peerconn.senddata(REPLY, 'Query ACK: %s' % port)
            if flag == "INIT":
                super(FilerPeer, self).setflag(True)
        except:
            self.__debug('invalid query %s: %s' % (str(peerconn), data))
            peerconn.senddata(ERROR, 'Query: incorrect arguments')
            
            
    def __handle_shutdown(self, peerconn, data):
        '''signal = data
        print "received data"
        if signal == 'INIT':
            BTPeer.setProcessOn( self )
        ...
        try:
            ...
            peerconn.senddata(REPLY, 'Query ACK: %s' % port)
            if flag == "SHUT":
                super(FilerPeer, self).setMaster(True)
                try:
                    fp = open('masterport.txt', 'w')
                    fp.write(str(ip)+":"+str(port))
                finally:
                    fp.close()
        except:
            self.__debug('invalid query %s: %s' % (str(peerconn), data))
